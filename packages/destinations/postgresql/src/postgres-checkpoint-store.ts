...
// of one schema, so dropping the schema resets both. Each run saves in its
// own transaction after the load commits; a crash in between replays the
// load, which deduplication absorbs.
export class PostgresCheckpointStore extends CheckpointStore {
  ...
  override async reset(id: string): Promise<void> {
    ...
    await this.#create(connection.sql);
    await connection.sql.begin(async (transaction) => {
      await this.#lock(transaction, id);
      await transaction.unsafe(`DELETE FROM ${this.#table} WHERE "id" = $1`, [
        id,
      ]);
    });
  }
  ...
  protected override async session<T>(
    ...
    await using connection = new Connection(this.#url, 'elt-checkpoints');
    await this.#create(connection.sql);
    const outcome = await connection.sql.begin(async (transaction) => {
      await this.#lock(transaction, id);
      return {
        value: await work({
          read: async () => {
            const [saved] = await transaction.unsafe(
  ...
  async #lock(transaction: postgres.TransactionSql, id: string): Promise<void> {
    const [lock] = await transaction.unsafe(
      'SELECT pg_try_advisory_xact_lock(hashtext($1), hashtext($2)) AS "locked"',
      [`mac-elt-checkpoints:${this.schema}`, id],
    );
    if (lock?.locked !== true)
