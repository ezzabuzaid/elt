...
| Changes | `EKEventStoreChangedNotification` invalidates previously fetched data. It supplies neither the durable cursor nor deletion records this pipeline needs for incremental reconciliation. Use full refresh. `Source.watch()` uses this notification only as a trigger to rerun full-refresh copies; see [watching for changes](reference.md#watching-for-changes). |
