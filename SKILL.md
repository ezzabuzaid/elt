...
- When one source instance reads several properties or accounts, declare `partitionKey` on the streams, list the partitions in `partitions(stream)` from configuration, and read the one `extract` receives. Keep partition values out of the source `identity`, and put the partition fields in every record and every primary key.
- Put source-specific selection rules in `validateExtraction()`, without I/O. Implement lazy `extract(configuration, state, partition)` yielding `{ stream, data }`, `DELETE` and `STATE` messages through the shared `Source.read()` path.
